...
webInterfaceGitUrl="https://github.com/pi-hole/AdminLTE/releases/tag/v5.13"
webInterfaceDir="${webroot}/admin"
piholeGitUrl="https://github.com/pi-hole/pi-hole/releases/tag/v5.11.4"
...
FTLinstall() {
    ...
    if [[ "${ftlBranch}" == "master" ]];then
        url="https://github.com/pi-hole/FTL/releases/tag/v5.16.3"
    else
...
FTLcheckUpdate() {
    ...
    if [[ ! "${ftlBranch}" == "master" ]]; then
        ...
        if [[ ${ftlLoc} ]]; then
            ...
            if ! FTLlatesttag=$(curl -sI https://github.com/pi-hole/FTL/releases/tag/v5.16.3 | grep --color=never -i Location: | awk -F / '{print $NF}' | tr -d '[:cntrl:]'); then
            ...
            if [[ "${FTLversion}" != "${FTLlatesttag}" ]]; then
                ...
                remoteSha1=$(curl -sSL --fail "https://github.com/pi-hole/FTL/releases/tag/v5.16.3/${FTLversion%$'\r'}/${binary}.sha1" | cut -d ' ' -f 1)
                localSha1=$(sha1sum "$(command -v pihole-FTL)" | cut -d ' ' -f 1)
